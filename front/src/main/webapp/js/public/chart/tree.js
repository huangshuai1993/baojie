define('echarts/chart/tree', [
    ...
    Tree.prototype = {
        ...
        _buildLink: function (parentNode, serie) {
            ...
            for (var i = 0; i < parentNode.children.length; i++) {
                ...
                case 'curve':
                    ...
                    break;
                case 'arrow':
                    this._buildBezierArrow(parentNode, parentNode.children[i], lineStyle, serie);
                    break;
                case 'broken':
                    break;
                default:
        ...
        _buildBezierArrow: function (parentNode, treeNode, lineStyle, serie) {
            var offsetRatio = GOLDEN_SECTION;
            var orient = serie.orient;
            var xStart = parentNode.layout.position[0];
            var yStart = parentNode.layout.position[1];
			...
			var s2e = Math.sqrt(Math.pow(xStart-xEnd, 2) + Math.pow(yStart-yEnd, 2));
			var t = 16.0 / s2e;
			
			var cpsx = xStart*Math.pow(1-t, 3) + 3*cpX1*t*Math.pow(1-t,2)
			            + 3*cpX2*Math.pow(t,2)*(1-t) + xEnd * Math.pow(t,3);
			var cpsy = yStart*Math.pow(1-t, 3) + 3*cpY1*t*Math.pow(1-t,2)
			            + 3*cpY2*Math.pow(t,2)*(1-t) + yEnd * Math.pow(t,3);
						
			var startD = Math.sqrt(Math.pow(xStart-cpsx, 2)+Math.pow(yStart-cpsy, 2));
			var sRotation = Math.asin((xStart-cpsx)/startD);

            var startArrowShape = new IconShape({
                style: {
                    x: -5,
                    y: 10,
                    width: 10,
            ...
            this.shapeList.push(startArrowShape);			

			t = 1-t;			
			var cpex = xStart*Math.pow(1-t, 3) + 3*cpX1*t*Math.pow(1-t,2)
			            + 3*cpX2*Math.pow(t,2)*(1-t) + xEnd * Math.pow(t,3);
			var cpey = yStart*Math.pow(1-t, 3) + 3*cpY1*t*Math.pow(1-t,2)
			            + 3*cpY2*Math.pow(t,2)*(1-t) + yEnd * Math.pow(t,3);
			
			var endD = Math.sqrt(Math.pow(xEnd-cpex, 2)+Math.pow(yEnd-cpey, 2));
			var eRotation = Math.PI + Math.asin((xEnd-cpex)/endD);
			
			var endArrowShape = new IconShape({
                style: {
                    x: -5,
                    y: 10,
                    width: 10,
            ...
            this.shapeList.push(endArrowShape);
        },
