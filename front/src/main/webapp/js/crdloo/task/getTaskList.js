$(function(){
	...
	$(".btn-distribution").on("click",function(){
		...
		if(storeId == 0){
			$("#region_task_modal option[value='0']").prop("selected","selected");
			$("#storeId_modal").html("<option value='0'>请选择</option>");
			$("#empId_modal").html("<option value='0'>请选择</option>");
		}else{
			$("#region_task_modal option[value='"+province+"']").prop("selected","selected");
  			var region_checkedval = province;
  			checkField(region_checkedval,'#storeId_modal');
  			$("#storeId_modal option[value='"+storeId+"']").prop("selected","selected");
  			storeChange(storeId,'#empId_modal');
		}
	...
	$("#saveTask").on("click",function(){
		...
		if(hasCautioner =='' || hasCautioner == undefined){
			...
			if(cautionerName == "" || cautionerIdcardNo=="" || cautionerRelation==""){
				...
				if(idcardNo == cautionerIdcardNo){
					alert("输入的担保人身份证号不能与客户重复");
	                return;
				}
