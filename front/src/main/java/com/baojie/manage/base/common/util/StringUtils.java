...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
			...
			if (args[i] != null) {
				...
				if (argNullable == false) {
					throw new IllegalArgumentException(
							MessageFormat.format("TextUtil.format arg cannot be null。 format:{0}, args:{1}", format,
	 ...
	 *
	 * @param format
	 *            格式, 例如book-{0}-{1}, {0}和{1}分别是第一个和第二个参数
	 * @param args
	 *            参数列表，必须和format里面的参数标识符个数一致， 参数值不能为null, 否则会抛出异常
	 * @return 格式化后的字符串
	 ...
	 * @return
	 */
	public static String realNameEncrpty(String realName) {
		return realName == null ? null
				: realName.replaceAll("([\\u4e00-\\u9fa5]{1}).*({1})",
						"$1*" + (realName.length() > 2 ? realName.charAt(realName.length() - 1) : ""));
	}
